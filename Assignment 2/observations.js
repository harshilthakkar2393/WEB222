...
function observationSummary2(data) {
  data.results.forEach((element) => {
    console.log(element.id + '@' + element.observed_on_details.date + ': ' + element.species_guess);
  });
}
...
function observationsByPrivacy(data, geoPrivacy) {
  var Arr = new Array(0);
  var correctValues = ['open', 'hidden', null];
  if (typeof geoPrivacy === 'string' || !geoPrivacy) {
    if (geoPrivacy) {
      geoPrivacy = geoPrivacy.toLowerCase();
    }
    if (correctValues.includes(geoPrivacy)) {
      for (let i = 0; i < data.results.length; i++) {
...
function transformObservation(original) {
  var arr = original.location.slice(',');
  var research = false;
  ...
  var newData = {
    ...
    isResearchQuality: research,
    geoCoords: [arr[1] * 1, arr[0] * 1],
    photoUrls: [original.url],
    photosCount: -1,
    user: '@' + original.user.login
  };
  newData.photosCount = newData.photoUrls.length;
  return newData;
}
...
function transformObservations(data) {
  var arr = [];
  data.results.forEach((element) => {
    arr.push(transformObservation(element));
  });
  return arr;
}
...
function getObservationsById(data, ...ids) {
  var arr = new Array(0);
  ids.forEach((element) => {
    if (data.results.find((result) => result.id === element)) {
      arr.push(data.results.find((result) => result.id === element));
    }
  });
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  ...
  if (options.greaterThan && options.lessThan && options.equal) {
    returnObservs = data.results.filter((element) => element.positional_accuracy === options.equal);
  } else if (options.greaterThan && options.lessThan) {
    if (options.greaterThan === options.lessThan) {
      returnObservs = new Array(0);
    } else {
      returnObservs = data.results.filter(
        (element) =>
    ...
    returnObservs = data.results;
  }
  return returnObservs;
}
  ...
  for (var i = 0; i < data.results.length; i++) {
    if (data.results[i].taxon) {
      if (data.results[i].taxon.default_photo !== null) {
        var url = data.results[i].taxon.default_photo.url;
        url = url.replace(/square.jpg*/g, '');
...
function getUserStats(data) {
  var Obs = 0,
    ...
    count = 0;
  for (var i = 0; i < data.results.length; i++) {
    Obs += data.results[i].user.observations_count;
    jour += data.results[i].user.journal_posts_count;
    spe += data.results[i].user.species_count;
    count = i + 1;
  }
  var obj = {
    count: count,
...
function extractUserLogins(data) {
  var arr = new Array();
  for (var i = 0; i < data.results.length; i++) {
    if (!arr.includes(data.results[i].user.login)) {
      arr.push(data.results[i].user.login);
    }
  }
  return arr;
}
...
function extractUserLogins2(data) {
  var dataSet = new Set();
  for (var i = 0; i < data.results.length; i++) {
    if (!dataSet.has(data.results[i].user.login)) {
      dataSet.add(data.results[i].user.login);
    }
  }
  var arr = Array.from(dataSet);
  return arr;
}
