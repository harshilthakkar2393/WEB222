...
function createVideo(src, loop, muted) {
  ...
  src = src.trim();
  tag = tag + src;
  if (loop === true && muted === true) {
    tag = tag + `" loop muted></video>`;
  } else if (loop === true) {
    tag = tag + `" loop></video>`;
  } else if (muted === true) {
    tag = tag + `" muted></video>`;
...
function fixPostalCode(postalCode) {
  ...
  if (
    !(
      /[^DFIOQWUZ]/.test(postalCode[0]) &&
      /[^DFIOQU]/.test(postalCode) &&
      /[0-9]/.test(postalCode[1]) &&
      /[0-9]/.test(postalCode[4]) &&
      /[0-9]/.test(postalCode[6])
    )
  ) {
    throw 'error';
  } else {
 ...
 * toProvince('M5W 1E6', true) returns ''
...
function toProvince(postalCode, useLongForm) {
  ...
  try {
    ...
    if (/^[KLMNP]/.test(postalCode)) {
      if (useLongForm) {
        returnValue = 'Ontario';
      } else {
        returnValue = 'ON';
      }
    } else if (/^[GHJ]/.test(postalCode)) {
      if (useLongForm) {
        returnValue = 'Quebec';
      } else {
        returnValue = 'QC';
      }
    } else if (/^B/.test(postalCode)) {
      if (useLongForm) {
        returnValue = 'Nova Scotia';
      } else {
        returnValue = 'NS';
      }
    } else if (/^E/.test(postalCode)) {
      if (useLongForm) {
        returnValue = 'New Brunswick';
      } else {
        returnValue = 'NB';
      }
    } else if (/^R/.test(postalCode)) {
      if (useLongForm) {
        returnValue = 'Manitoba';
      } else {
        returnValue = 'MB';
      }
    } else if (/^V/.test(postalCode)) {
      if (useLongForm) {
        returnValue = 'British Columbia';
      } else {
        returnValue = 'BC';
      }
    } else if (/^C/.test(postalCode)) {
      if (useLongForm) {
    ...
    } else if (/^T/.test(postalCode)) {
      if (useLongForm) {
        returnValue = 'Alberta';
      } else {
        returnValue = 'AB';
      }
    } else if (/^A/.test(postalCode)) {
      if (useLongForm) {
        returnValue = 'Newfoundland and Labrador';
      } else {
        returnValue = 'NL';
      }
    } else if (/^X/.test(postalCode)) {
      if (useLongForm) {
        returnValue = 'Northwest Territories and Nunavut';
      } else {
        returnValue = 'NT';
      }
    } else if (/^Y/.test(postalCode)) {
      if (useLongForm) {
        returnValue = 'Yukon';
      } else {
        returnValue = 'YT';
...
function normalizeCoord(value) {
  ...
  if (
    !(
      parseFloat(coord[0]) > -91 &&
      parseFloat(coord[0]) < 91 &&
      parseFloat(coord[1]) < 181 &&
      parseFloat(coord[1]) > -181
    )
  ) {
...
function formatCoords(...values) {
  var returnValues, coordinates;
  for (var i = 0; i < values.length; i++) {
    try {
      if (i === 0) {
        coordinates = normalizeCoord(values[i]);
      } else {
        coordinates += ', ' + normalizeCoord(values[i]);
      }
    } catch (error) {
  ...
  if (postalCodes.length < 1) {
    ...
    for (var i = 0; i < postalCodes.length; i++) {
      ...
      if (provinceCode.length > 2) {
        postalCodes[i] = toProvince(postalCodes[i], 1);
      } else {
        postalCodes[i] = toProvince(postalCodes[i], 0);
      }
      if (postalCodes[i] === provinceCode) {
        total++;
      }
    }
    return total;
...
function buildQuery(query, perPage, page, format) {
  var uri = 'https://api.web222-example.org/v3/query';
  if (
    !(
      typeof query === 'string' &&
      perPage < 101 &&
      perPage > 0 &&
      page > 0 &&
      (format === 'json' || format === 'html' || format === 'xml')
    )
  ) {
    throw 'error';
  } else {
    uri = uri + `?query=${query}&per_page=${perPage}&page=${page}&format=${format}`;
    return uri;
  }
}
